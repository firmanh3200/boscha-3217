...
st.title('Produk Domestik Regional Bruto (PDRB) Kabupaten Bandung Barat')
...
with st.container(border=True):
    st.success('Tren PDRB Kabupaten Bandung Barat (Milyar Rupiah)')
...
with tab1a:
    with st.container(border=True):
        st.success('Tren PDRB Berlaku menurut Lapangan Usaha Kabupaten Bandung Barat (Milyar Rupiah)')
...
with tab1b:
    with st.container(border=True):
        st.success('Tren PDRB Konstan menurut Lapangan Usaha Kabupaten Bandung Barat (Milyar Rupiah)')
...
with tab2a:
    with st.container(border=True):
        st.success('Tren PDRB Berlaku menurut Pengeluaran Kabupaten Bandung Barat (Milyar Rupiah)')
...
with tab2b:
    with st.container(border=True):
        st.success('Tren PDRB Konstan menurut Pengeluaran Kabupaten Bandung Barat (Milyar Rupiah)')
...
with st.expander('Lihat Tabel Lengkap PDRB menurut Lapangan Usaha'):
    st.warning('Produk Domestik Regional Bruto menurut Lapangan Usaha Kabupaten Bandung Barat')
    df = data.sort_values(by='Tahun', ascending=False)
    st.dataframe(df, hide_index=True, use_container_width=True)
    
with st.expander('Lihat Tabel Lengkap PDRB menurut Pengeluaran'):
    st.warning('Produk Domestik Regional Bruto menurut Pengeluaran Kabupaten Bandung Barat')
    df = pdrbx.sort_values(by='Tahun', ascending=False)
    st.dataframe(df, hide_index=True, use_container_width=True)
    
st.subheader('', divider='rainbow')
st.caption(':orange[Boscha - Statistik Daerah Kabupaten Bandung Barat]')
st.caption('Hak Cipta @ :green[BPS Kabupaten Bandung Barat]')